...
// переводы строк в \n. Тело таски в блок не попадает и через YAML не проходит:
// парсер получает ~1 КБ фронтматтера вместо ~4 КБ файла.
//
// Файл при этом читается целиком — вызывающий код передаёт готовый src.
// Обрывать чтение на фенсе смысла нет: все 1264 таски вместе весят 5.19 МБ,
// и экономия ~4 МБ разового ввода-вывода при старте не окупает кода.
